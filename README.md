...
### - LogInView.java와 SignUpView.java:
- LogInView.java는 사용자가 웹 DB의 회원 정보를 바탕으로 로그인할 수 있는 기능을 제공합니다. JSON 데이터를 파싱하여 사용자 입력값을 검증하고, 로그인 성공 시 MainActivity로 이동합니다.  
- SignUpView.java는 사용자가 회원가입을 할 수 있도록 하며, 입력된 데이터는 웹 DB에 저장됩니다.  
